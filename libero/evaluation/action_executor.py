...
class ActionChunkExecutor:
    ...
    def __init__(
        ...
        capture_action_metadata: bool = False,
    ):
        ...
        self.capture_action_metadata = bool(capture_action_metadata)
        self._queue: Deque[np.ndarray] = deque()
        self._metadata_queue: Optional[Deque[dict]] = (
            deque() if self.capture_action_metadata else None
        )
        self._episode_id = ""
        self._instruction = ""
    ...
    def act(self, obs, step: int) -> np.ndarray:
        ...
        if not self._queue:
            request = PolicyRequest(
                ...
                capture_diagnostics=self.capture_action_metadata,
            )
            import time
            ...
            if self.capture_action_metadata:
                base_metadata = dict(response.metadata)
                base_metadata.update(
                    {
                        "policy_query_index": self.query_count - 1,
